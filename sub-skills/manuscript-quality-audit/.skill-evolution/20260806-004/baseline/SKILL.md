...
由多个独立审读任务分别检查情绪兑现与叙事体验；自动脚本检查缺章、标题、用户词表命中、重复段落和高频句首。将结果合并至 审校/最终/quality-report.md。

禁用词表只使用 项目/禁用词.txt 中作者/平台明确要求；结果是命中清单而非自动删改。所有 S1、未解决 S2、例外批准和开放项必须进入总结报告。
